...
class starter {
		...
		*/
		while(true){
			...
			if (choice.equals("check")){
				...
				System.out.println("There are " + checksArray(arr, phrase) + " many '" + phrase + "' in " + textFile);
				
			}
			if (choice.equals("common")){
				System.out.println(mostCommon(arr) + " is the most common word");
			}
	...
	*/
	public static int checksArray(String[] arr, String check){
		...
		for (int i = 0; i < arr.length - check.length(); i++){
			String word = whole.substring(i, check.length() + i);
